        ...
        if ( $retArray ) {
            ...
            if ( $args['columns'] == 3 && $numRows > 2 ) {
                ...
                for ( $i = 1; $i <= $numRows; $i++ ) {
                    ...
                    // fgabriel: adding code to filter by page name (the extension didn't allow this by default)
                    $show = true;
                    if (isset($GLOBALS['wgExcludeFromRecentPages'])) {
                      global $wgExcludeFromRecentPages;
                      
                      foreach($wgExcludeFromRecentPages as $word) {
                        $hasWord = (strpos(strtolower($title['page_title']), strtolower($word)) !== false);
                        if ($hasWord) {
                          $show = false;
                          break;
                        }
                    ...
                    // fgabriel: end of code for filtering by name
